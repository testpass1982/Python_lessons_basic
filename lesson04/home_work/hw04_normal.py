...
print('tesk1 with regexp:')
import re

result = re.sub('[A-Z]{1,}', " ", line).split(" ")
print(result)

print('task1 without regexp:')
...
line_2 = 'mtMmEZUOmcqWiryMQhhxTqKdSTKCYEJlEZCsGAMkgAYEOmHBSQsSUHKvSfbmxULaysm' \
...
# without re
print("task2 without re:")
i = 2
result = []
while i < len(line_2):
    if line_2[i].isupper() and line_2[i - 1].islower() and line_2[i - 2].islower():
        j = i
        word = ''
        while line_2[j].islower() != True and j < len(line_2) - 2:
            word += line_2[j]
            j += 1
